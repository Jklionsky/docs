...
<Note>
  ...
  with a one-credit minimum when no deals match. `GET /alerts/configurations` is
  free. See [Pricing and Credits](/usage).
</Note>
...
also includes reasoning that explains why the deal satisfied the alert and an
`articles` array containing its related sources.

Monitor `meta.credits_used` and use narrower date ranges or fewer alert IDs when
you need to limit credit consumption.
