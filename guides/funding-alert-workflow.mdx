...
<Note>
  `GET /alerts` and `GET /alerts/configurations` are available on Pro+,
  Enterprise, and API plans. Both endpoints consume zero credits. See
  [Pricing and Credits](/usage).
</Note>
...
Use narrower date ranges or fewer alert IDs to keep response payloads smaller and
downstream processing faster.
