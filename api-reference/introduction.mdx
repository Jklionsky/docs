...
### Alerts
- **GET /alerts** -- Retrieve saved alert data with all matching deals in a date range (up to 10 alerts per request). Deals are deduplicated, sorted newest-first, and each includes AI-generated reasoning and related source articles.
- **GET /alerts/configurations** -- List the authenticated user's alert configurations (filters, frequency, descriptions).

`GET /alerts` consumes one credit per deal returned across all requested alerts,
with a one-credit minimum when no deals match. `GET /alerts/configurations` is free.
